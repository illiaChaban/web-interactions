 ...
 */
export const animateOnScroll = (callback: (y: number) => void) => {
  ...
  const animate = () => {
    requestAnimationFrameId = requestAnimationFrame(() => {
      callback(target instanceof Window ? target.scrollY : target.scrollTop);
      animate();
    });
  };
  ...
  const handleScroll = (e: Event) => {
    target = e.currentTarget as HTMLElement | Window;
    const isFirstFrame = !lastScrollEventTime;
