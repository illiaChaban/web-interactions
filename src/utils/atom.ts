import { createSignal, Accessor, Setter, SignalOptions } from "solid-js";
...
type UseAtom = {
   ...
   * With an argument acts like a setter.
   */
  <T>(defaulValue: T, options?: SignalOptions<T>): Atom<T>;
};
export const atom: UseAtom = <T>(
  defaultValue?: T,
  options?: SignalOptions<T>
) => {
  const [value, setValue] = createSignal(defaultValue as any, options);
